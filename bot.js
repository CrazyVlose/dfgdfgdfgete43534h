...
if(message.content === `${prefix}voiceonline`) {
  message.guild.createChannel(`Orexn voice : ${message.guild.members.filter(g => g.voiceChannel).size} ` , "voice").then(c => {
   c.overwritePermissions(message.guild.id, {CONNECT: false});
    message.channel.send(`**Orexn voice : ${message.guild.members.filter(g => g.voiceChannel).size}**`);
    setInterval(() => {
...
client.on("ready", () => {
    var guild;
    while (!guild)
        guild = client.guilds.get("508761735365263370");
...
client.on("guildMemberAdd", (member) => {
    let channel = member.guild.channels.get("508765601636810768");
    if (!channel) {
...
client.on("guildMemberAdd", member => {
let welcomer = member.guild.channels.find("name","text");
      if(!welcomer) return;
      if(welcomer) {
         moment.locale('ar-ly');
         var h = member.user;
